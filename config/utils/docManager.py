...
def getTemplateImageUrl(fileType, request):
    ...
    return mapping.get(fileType, path + 'file_docx.svg')
...
def getCorrectName(filename, req):
    ...
    i = 1
    # if file with such a name already exists
...
def getHistoryPath(filename, file, version, req):
    ...
    if not os.path.exists(directory):
        filePath = os.path.join(getRootFolder(
            req), f'{filename}-hist', version, file)
    else:
...
def getForcesavePath(filename, req, create):
    ...
    # and get the path to the given file
...
def getStoredFiles(req):
    ...
    files.sort(key=lambda x: os.path.getmtime(os.path.join(directory, x)),
               reverse=True)  # sort files by time of last modification

    fileInfos = []

    for f in files:
        if os.path.isfile(os.path.join(directory, f)):
            fileInfos.append({'isFillFormDoc': isCanFillForms(fileUtils.getFileExt(f)), 'version': historyManager.getFileVersion(historyManager.getHistoryDir(getStoragePath(
                # write information about file type, title and url
                f, req))), 'type': fileUtils.getFileType(f), 'title': f, 'url': getFileUri(f, True, req), 'canEdit': isCanEdit(fileUtils.getFileExt(f))})
...
def createFile(stream, path, req=None, meta=False):
    ...
    with io.open(path, 'wb') as out:  # write data to the file by streams
        ...
        while len(read) > 0:
            ...
            read = stream.read(bufSize)
    if meta:
        # create meta data for the file if needed
...
def downloadFileFromUri(uri, path=None, withSave=False):
    resp = requests.get(
        uri, stream=True, verify=config_manager.ssl_verify_peer_mode_enabled(), timeout=5)
    status_code = resp.status_code
    if status_code != 200:  # checking status code
        raise RuntimeError(
            'Document editing service returned status: %s' % status_code)
    if withSave:
        if path is None:
...
def createSample(fileType, sample, req):
    # get the internal extension of the given file type
    ext = getInternalExtension(fileType)

    if not sample:
        sample = 'false'

    # create sample or new template
    ...
    with io.open(os.path.join('assets', 'document-templates', 'sample' if sample == 'true' else 'new', f'{sampleName}{ext}'), 'rb') as stream:
...
def getFilesInfo(req):
    ...
    for f in getStoredFiles(req):  # run through all the files from the directory
        ...
        if fileId:  # if file id is defined
            # and it is equal to the file key value
            if fileId == generateFileKey(f.get("title"), req):
                # add file object to the response array
